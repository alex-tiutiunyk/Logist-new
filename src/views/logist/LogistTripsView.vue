...
onMounted(async () => {
  await Promise.all([tripsStore.fetchTrips(), loadDrivers()])
})
...
async function createTrip() {
  if (!newTrip.value.number) { notifStore.error('Введіть номер рейсу'); return }
  ...
  if (result.success) {
    ...
    notifStore.error('Помилка створення')
  }
  ...
  <div class="flex flex-col gap-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted mt-0.5">{{ filteredTrips.length }} рейсів</p>
      </div>
      <AppButton variant="primary" @click="showCreateModal = true">+ Новий рейс</AppButton>
    </div>

    <div class="flex gap-3 flex-wrap">
      <input v-model="search" type="text" placeholder="Пошук..." class="px-3 py-2 rounded-lg border border-border text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-primary/30 w-64" />
      <select v-model="filterStatus" class="px-3 py-2 rounded-lg border border-border text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-primary/30">
    ...
    <div class="bg-surface border border-border rounded-xl overflow-hidden">
      <div v-if="tripsStore.isLoading" class="flex justify-center py-8"><AppSpinner /></div>
      <div v-else-if="filteredTrips.length === 0" class="text-center text-muted text-sm py-8">Рейсів не знайдено</div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-border">
            <tr
              ...
              @click="router.push({ name: 'logist-trip-detail', params: { id: trip.id } })"
            >
              ...
              <td class="px-6 py-4 flex items-center gap-3">
                <button class="text-primary hover:underline text-xs" @click.stop="router.push({ name: 'logist-trip-detail', params: { id: trip.id } })">Деталі</button>
                <button class="text-red-500 hover:underline text-xs" @click="confirmDelete(trip, $event)">Видалити</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <AppModal v-model="showCreateModal" title="Новий рейс" persistent>
      <form class="flex flex-col gap-4" @submit.prevent="createTrip">
        ...
        <div class="flex items-center gap-2">
          <input v-model="newTrip.isInternational" type="checkbox" id="logistIntl" class="rounded" />
          <label for="logistIntl" class="text-sm text-gray-700">Міжнародний рейс</label>
        </div>
      </form>
      <template #footer>
    ...
    <AppModal v-model="showDeleteConfirm" title="Видалити рейс?">
      <p class="text-sm text-gray-700">
        Ви впевнені, що хочете видалити рейс <strong>{{ deletingTrip?.number }}</strong>?
      </p>
      <template #footer>
        <div class="flex gap-2 justify-end">
