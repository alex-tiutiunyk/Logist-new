...
const editForm = ref({
  number: '', driverUid: '', eta: '', isInternational: false, originName: '', destinationName: '',
})
...
async function confirmEdit() {
  ...
  if (result.success) {
    await eventsStore.addEvent({
      ...
      by: { uid: authStore.user?.uid, name: authStore.user?.displayName || 'Логіст' },
    })
...
const containerTypeLabels = { '20ft': '20 футів', '40ft': '40 футів', '40ft_hc': '40 футів HC', '45ft': '45 футів' }
...
<template>
  <div class="flex flex-col gap-6">
    <div v-if="tripsStore.isLoading && !trip" class="flex justify-center py-12"><AppSpinner size="lg" /></div>

    <template v-else-if="trip">
      <div v-if="isSos" class="flex items-center gap-3 px-5 py-3 rounded-xl bg-red-600 text-white animate-pulse">
      ...
      <div class="flex border-b border-border gap-4">
        <button
          v-for="tab in [{ key: 'info', label: 'Інформація' }, { key: 'timeline', label: 'Хронологія' }, { key: 'containers', label: 'Контейнери' }]"
          :key="tab.key"
      ...
      <div v-if="activeTab === 'info'" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="bg-surface border border-border rounded-xl p-5">
          ...
          <dl class="flex flex-col gap-2 text-sm">
            <div class="flex justify-between"><dt class="text-muted">Водій</dt><dd class="font-medium">{{ trip.driverName || '—' }}</dd></div>
            <div class="flex justify-between"><dt class="text-muted">ETA</dt><dd class="font-medium">{{ formatDate(trip.eta) }}</dd></div>
            <div class="flex justify-between"><dt class="text-muted">Тип</dt><dd class="font-medium">{{ trip.isInternational ? 'Міжнародний' : 'Внутрішній' }}</dd></div>
            <div class="flex justify-between"><dt class="text-muted">Створено</dt><dd class="font-medium">{{ formatDate(trip.createdAt) }}</dd></div>
            <div v-if="isSos" class="flex justify-between mt-2 pt-2 border-t border-red-200">
              <dt class="text-red-600 font-semibold">SOS статус</dt>
        ...
        <div class="bg-surface border border-border rounded-xl p-5">
          ...
          <div class="flex flex-col gap-2">
            <div v-for="(wp, idx) in trip.waypoints" :key="wp.id || idx" class="flex items-center gap-2 text-sm">
              <div class="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold" :class="wp.completed ? 'bg-accent text-white' : 'bg-border text-muted'">{{ idx + 1 }}</div>
    ...
    <div v-else class="text-center text-muted py-12">Рейс не знайдено</div>
    ...
    <AppModal v-model="showEditModal" title="Редагувати рейс" persistent>
      <form class="flex flex-col gap-4" @submit.prevent="requestEditConfirm">
        ...
        <div class="flex items-center gap-2">
          <input v-model="editForm.isInternational" type="checkbox" id="logistEditIntl" class="rounded" />
          <label for="logistEditIntl" class="text-sm text-gray-700">Міжнародний рейс</label>
        </div>
      </form>
      <template #footer>
    ...
    <AppModal v-model="showEditConfirm" title="Підтвердити зміни?">
      <p class="text-sm text-gray-700">Зберегти зміни до рейсу <strong>{{ editForm.number }}</strong>? Рейс є активним.</p>
      <template #footer>
        <div class="flex gap-2 justify-end">
    ...
    <AppModal v-model="showDeleteConfirm" title="Видалити рейс?">
      <p class="text-sm text-gray-700">Ви впевнені, що хочете видалити рейс <strong>{{ trip?.number }}</strong>?</p>
      <template #footer>
        <div class="flex gap-2 justify-end">
